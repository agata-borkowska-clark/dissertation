\documentclass[12pt,journal,duplex]{IEEEtran}
%\usepackage{hyperref}
...
\begin{document}
	...
	With this firm rooting, we then further refine and decide on the scope of the project, since our work is limited by time constraints independent from us.
	...
	We then test our findings against a different, more complicated scenario, to see how much we have learnt.
	...
	Multiple plug-ins were created to help Atelier B provers with discharging proof obligations, however they are small projects which do not go through as strict accreditation process as Atelier B itself. However their existence highlights that the original prover leaves some to be desired. We do not want to use them in this work, because it is aimed at people new to the B method and we consider it to be too much of a complication of the proof process overall, to work with additional software.\cite{SMT}\cite{BEval}\cite{BWare}
	
	One program we intend to use however, is ProB, software designed to animate Atelier B models by simulating the operations on the machine with the values of the users' choosing. We have found it very helpful while learning to formalize specification correctly and checking for simple human errors.\cite{proB}
	...
	An extension to this aim is answering the question: at what point is it necessary to add user-created rules in order to facilitate the proofs in the automated prover? It needs to be stressed that adding user-created rules is not advised unless it is absolutely necessary - and every manual as well as the works discussing methods of validating such rules accentuate this point. The rules then have to be thoroughly verified by means other software or manual proof, to ensure that they are in fact correct. An error in an added rule would invalidate the entire proof process. Hence we will strive to circumvent obstacles by means other than adding proof rules, before resorting to it. We hope to gather such advice and guidelines and make it accessible to others, so that they may be dissuaded from unnecessarily adding proof rules in their work. It is possible that adding proof rules to the Atelier B's rule base may be avoided entirely in the scenarios we have chosen, as they are by no means exhaustive; otherwise we will present the rules and their proofs for the use of others who wish to study the B-method.

	The observations arising from our work can be separated into two groups. Firstly there will be points highlighting the intricacies of the Atelier B software. 	Secondly, there will be disparities between the B method and its implementation in the Atelier B software. Not all of them can be classified as bugs, and some are clearly conscious choices which diverge from the pure theory of the B method. The manuals provided with the software will aid us with comparing the implementation to the theory. They are:
	...
	\section{Methodology}
	\subsection{Overview}
	We will focus on the B method created by Abrial, as it is implemented in the Atelier B software, which was developed by ClearSy Ltd. The software is available on ClearSy's website. The version of Atelier B which we will be using, is 4.2.1.
	...
	We take on a slightly informal approach, to demonstrate an intuition behind the proof process employing the itneractive prover.
...
pr & pp(rp.1) & pr
	...
	\begin{tabular}{@{}lcc@{}}
		\toprule
		 Machine                     & \multicolumn{1}{l}{\begin{tabular}[c]{@{}l@{}}Number \\ of POs\end{tabular}} & \multicolumn{1}{l}{\begin{tabular}[c]{@{}l@{}}Of which\\ undischarged\end{tabular}} \\ \midrule
...
\end{table}
	...
	Combining the observations from both sets of machines, we have established the following approach.
		...
		\subsubsection{}	Well-definess proof obligations in our experience are the main source of pointers towards errors in the formalisation of the specification. We advise to inspect them first. If they are satisfied, but the prover struggles to demonstrate them, they can be added as conjuncts in the Invariant, at the cost of additional proof obligations regarding the preservation of the Invariant. 
		
		\subsubsection{} Inspecting the well-defineness proof obligations also helps to establish how the prover normalises certain expressions. The user may consider rewriting them as they appear in the interactive prover, to reduce the number of rewrite rules applied by the automated prover in the background.
		
		\subsubsection{} The remaining proof obligations can be tackled with the predicate prover, although it should be noted that its operation requires certain amount of resources, and the prover may run out of time or dedicated memory. Sometimes repeated attempts can lead to a success.
		
		\subsubsection{} Another option is to add hypothesis to guide the prover along. The hypothesis should be provable from what is already known, so cannot include any large leaps. Then proving the goal from the hypothesis should be equally simple. Quite often the added hypotheses are slight rephrasing of the Invariant or the preconditions of an operation, that will imply the goal of the proof obligation.


	We hope that this summarises the intuition behind our method sufficiently for others to follow it.
